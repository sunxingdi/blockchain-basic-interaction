...
import { usdt_abi } from "./contract_abi";
...
//转账额度
let transfer_amount: number = 100; // 准备转移100个USDT
//USDT精度
let usdt_decimals: number = 6;
...
async function initialContract() {
  ...
  console.log("\n", '初始化合约...');
  usdt_contract   = new ethers.Contract(usdt_address,   usdt_abi,   wallet_A); //如果要发送交易，需要用私钥初始化

  console.log('USDT合约名称：', await usdt_contract.name());
  // usdt_decimals = await usdt_contract.decimals()
  console.log('USDT合约精度：', usdt_decimals);
...
async function makeTransaction() {
  ...
  const transfer_txn = await usdt_contract.transfer(
    wallet_B.address,                 // 函数参数: _to (收款地址)
    transfer_amount * Math.pow(10, usdt_decimals), // 函数参数:_value (转账金额)
    overrides                        // 交易参数
  )
  ...
  console.log("\n", '等待上链...');
  await transfer_txn.wait();

  console.log("\n", '获取交易回执...');
  const receipt = await provider.getTransactionReceipt(transfer_txn.hash);
  console.log(receipt);
